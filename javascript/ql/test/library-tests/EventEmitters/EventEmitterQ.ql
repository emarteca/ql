...
string aggregateListOfBrokenAPIUses(ListenNode ln) {
  result = concat( Portal p, string s | ln.getBase*() = p.getAnExitNode(_) and s = incorrectAPIUse(p, ln.getEventName()) | 
          "[ " + p + ", " + s + " ]\n"
  )
}
...
//select s, ln, p, ln.getEventName() //, ln.getReceiver(),fn, fn.getNumParameter() , getListOfParams_FunctionNode(fn)
